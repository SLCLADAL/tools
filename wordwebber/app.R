# ============================================================
#  WordNetter — LADAL Word Co-occurrence Network Tool
#  https://ladal.edu.au
#
#  Pipeline:
#    .txt files → corpus → tokens (optional stopword removal)
#    → FCM (sliding window, tri=TRUE) → sparse keyword select
#    → small dense submatrix → static textplot_network
#                             + interactive visNetwork
#
#  Performance fixes vs original WordMapper:
#    1. All heavy computation moved into eventReactive so it only
#       fires on button click, never on slider/dropdown changes.
#    2. fcm_select() extracts a sparse keyword neighbourhood
#       BEFORE any as.matrix() call, avoiding the V×V dense
#       matrix that caused OOM crashes on real corpora.
#    3. tri=TRUE halves FCM storage; neighbourhood_fcm reuses
#       the already-computed edge list instead of calling
#       as.matrix() a second time.
#    4. visNetwork stabilization capped at 100 iterations.
# ============================================================
...
# Build FCM.
# FIX: tri=TRUE — store upper triangle only, halving memory.
# FIX: this function is called only from inside eventReactive,
#      never from a plain reactive(), so it never fires on slider changes.
build_fcm <- function(corp, window_size, stopword_lang,
  ...
  fcm(toks,
      ...
      window  = window_size,
      ordered = FALSE,
      tri     = TRUE)          # FIX: was FALSE — upper-tri is sufficient
}

# Extract co-occurrence edges for a keyword.
#
# FIX: uses fcm_select() to get a tiny sparse submatrix containing only
# the keyword and its neighbours BEFORE calling as.matrix(). This avoids
# materialising the full V×V dense matrix which caused OOM on real corpora.
keyword_cooc <- function(fcm_obj, keyword, top_n, min_freq) {
  ...
  # Select only the keyword row/col from the sparse FCM — O(k) not O(V²)
  kw_fcm <- fcm_select(fcm_obj, pattern = kw, selection = "keep",
                        valuetype = "fixed", verbose = FALSE)

  # Convert this tiny single-row matrix to a named vector
  kw_vec <- as.matrix(kw_fcm)[kw, , drop = TRUE]

  # FCM with tri=TRUE: values may be in upper triangle only.
  # We also need to check the column for the keyword in the full FCM
  # (for words that appear BEFORE the keyword in the upper-tri layout).
  # fcm_select with the keyword gives us both by fetching the full row.
  # But for tri=TRUE the column entries are stored as row entries for the
  # other word. We grab the column too using a second select.
  col_fcm <- fcm_select(fcm_obj, pattern = kw, selection = "keep",
                         valuetype = "fixed", verbose = FALSE)
  col_vec <- as.matrix(col_fcm)[, kw, drop = TRUE]

  # Merge: sum the row and column vectors to get symmetric counts
  all_words <- union(names(kw_vec), names(col_vec))
  cooc_vec  <- setNames(numeric(length(all_words)), all_words)
  cooc_vec[names(kw_vec)] <- cooc_vec[names(kw_vec)] + kw_vec
  cooc_vec[names(col_vec)] <- cooc_vec[names(col_vec)] + col_vec

  # Remove self and zero entries
...
# Build the small sub-FCM for the keyword neighbourhood.
#
# FIX: reuses the edge list already computed by keyword_cooc() to
# select only the neighbourhood words, then calls fcm_select() on
# those — a small sparse op. No second full as.matrix() call.
neighbourhood_fcm <- function(fcm_obj, keyword, edges) {
  if (is.null(edges) || nrow(edges) == 0) return(NULL)

  nodes_keep <- unique(c(edges$from, edges$to))

  # Select neighbourhood submatrix (sparse)
  sub_fcm <- fcm_select(fcm_obj,
                         pattern   = nodes_keep,
                         selection = "keep",
                         valuetype = "fixed",
                         verbose   = FALSE)

  # Make symmetric: add upper and lower triangles
  # This is safe on the small neighbourhood matrix (top_n × top_n)
  sub_mat <- as.matrix(sub_fcm)
...
ui <- fluidPage(
  ...
  tags$head(tags$style(HTML(paste0("
    ...
    .wn-banner .wn-title { font-size:1.7rem; font-weight:700;
                            letter-spacing:.5px; margin:0; }
    .wn-banner .wn-sub   { font-size:.88rem; opacity:.85; margin:2px 0 0 0; }

    /* Layout */
    .wn-body { display:flex; min-height:calc(100vh - 80px); }
    .wn-side  { width:310px; min-width:260px; max-width:340px;
                background:white;
                border-right:1px solid #e0d8ec;
                padding:22px 20px 30px 20px;
                box-shadow:2px 0 8px rgba(81,36,122,.06); }
    .wn-main  { flex:1; padding:24px 28px; overflow-x:auto; }
    ...
    .wn-sec:first-child { margin-top:0; }
    ...
    .wn-stats { display:flex; gap:12px; margin-bottom:20px;
                flex-wrap:wrap; }
    .wn-card  { background:white; border-radius:9px;
                border-left:4px solid ", LADAL_PURPLE, ";
                padding:11px 16px; min-width:110px;
                box-shadow:0 1px 6px rgba(81,36,122,.08); }
    .wn-card .wn-val { font-size:1.5rem; font-weight:700;
                       color:", LADAL_PURPLE, "; line-height:1.1; }
    .wn-card .wn-lbl { font-size:.76rem; color:#888; margin-top:2px; }
    ...
    .wn-dl:hover { background:", LADAL_PURPLE, "; color:white; }
  ...
  div(class = "wn-body",
    ...
    div(class = "wn-side",
      ...
      # Visual options
      div(class = "wn-sec", "④ Visual options"),
  ...
  # ── Footer ───────────────────────────────────────────────────
  div(class = "wn-footer",
    span("WordNetter · LADAL · University of Queensland"),
...
server <- function(input, output, session) {
  ...
  # ── Core computation — only fires on button click ───────────
  #
  # FIX: was two separate reactives (fcm_obj + net_data).
  # fcm_obj was a plain reactive() that fired every time
  # window_size or stopword_lang changed, re-tokenising and
  # re-building the FCM on every slider touch.
  # Everything is now inside one eventReactive so nothing
  # expensive runs until the user explicitly clicks the button.
  net_data <- eventReactive(input$run_network, {
    ...
    withProgress(message = "Building network…", value = 0, {
      ...
      incProgress(0.2, detail = "Tokenising corpus")
      fcm_obj <- build_fcm(corp(),
      ...
      incProgress(0.3, detail = "Preparing graph")
      # FIX: pass edges to neighbourhood_fcm so it doesn't
      # recompute from scratch with a second as.matrix() call
      sub_fcm <- neighbourhood_fcm(fcm_obj, kw, ed)
  ...
  output$corpus_status <- renderUI({
    if (is.null(input$files)) {
      div(class = "wn-warn", "⚠ No files uploaded yet.")
    } else {
      n   <- nrow(input$files)
  ...
  output$welcome_box <- renderUI({
    if (input$run_network == 0) {
      div(class = "wn-info", style = "font-size:.93rem;",
        tags$b("Welcome to WordNetter."), br(),
  ...
  output$stats_cards <- renderUI({
    ...
    div(class = "wn-stats",
      div(class = "wn-card",
        div(class = "wn-val", nrow(nd$edges)),
        div(class = "wn-lbl", "Co-occurring words")),
      div(class = "wn-card",
        div(class = "wn-val", sum(nd$edges$cooc)),
        div(class = "wn-lbl", "Total co-occurrences")),
      div(class = "wn-card",
        div(class = "wn-val", nd$edges$cooc[1]),
        div(class = "wn-lbl", paste0("Top: '", nd$edges$to[1], "'"))),
      div(class = "wn-card",
        div(class = "wn-val", format(nd$n_tokens, big.mark = ",")),
        div(class = "wn-lbl", "Corpus tokens"))
    )
  })
  ...
  output$vis_net <- renderVisNetwork({
    ...
    visNetwork::visNetwork(
      ...
      visNetwork::visPhysics(
        ...
        # FIX: was 200 — 100 is sufficient and avoids browser freeze
  ...
  output$static_net <- renderPlot({
    ...
    quanteda.textplots::textplot_network(
      x              = sub_fcm,
      min_freq       = input$min_freq / max(sum(as.matrix(sub_fcm)), 1),
  ...
  output$cooc_table <- renderDT({
    ...
    formatStyle(
      "CoocFreq",
      background         = styleColorBar(nd$edges$cooc, "#d8c8f0"),
  ...
  output$download_buttons <- renderUI({
    ...
    tagList(
      downloadButton("dl_png",  "⬇ Network (.png)", class = "wn-dl"),
      downloadButton("dl_xlsx", "⬇ Table (.xlsx)",  class = "wn-dl"),
      downloadButton("dl_csv",  "⬇ Table (.csv)",   class = "wn-dl")
    )
  })
  ...
  output$dl_xlsx <- downloadHandler(
    filename = function()
      paste0("wordnetter_", net_data()$keyword, "_", Sys.Date(), ".xlsx"),
  ...
  output$dl_csv <- downloadHandler(
    filename = function()
      paste0("wordnetter_", net_data()$keyword, "_", Sys.Date(), ".csv"),
    content  = function(file)
  ...
  output$dl_png <- downloadHandler(
    filename = function()
      paste0("wordnetter_", net_data()$keyword, "_", Sys.Date(), ".png"),
    content  = function(file) {
      nd      <- net_data()
      req(!is.null(nd))
      kw      <- nd$keyword
