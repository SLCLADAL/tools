...
library(visNetwork)
library(ggplot2)
library(textstem)     # lemmatise_words() — English only, no model download
...
# Extract co-occurrence edges for a keyword using the sparse FCM.
# Returns a tibble with raw count (cooc) and MI score per collocate.
keyword_cooc <- function(fcm_obj, freq_vec, keyword, top_n, min_freq) {
  ...
  # Select the keyword's row from the symmetric sparse FCM.
  # With tri=FALSE this row contains ALL co-occurrences for the keyword.
  kw_fcm <- fcm_select(fcm_obj,
                        pattern   = kw,
                        selection = "keep",
                        valuetype = "fixed",
                        verbose   = FALSE)

  if (nrow(kw_fcm) == 0L) return(NULL)

  kw_vec <- as.matrix(kw_fcm)[kw, , drop = TRUE]

  # Remove self-co-occurrence and entries below threshold
  ...
  # N_pairs: total co-occurrence slots in the FCM
  N_pairs <- sum(fcm_obj)
...
neighbourhood_fcm <- function(fcm_obj, edges) {
  ...
  # Symmetrise the small submatrix (safe at top_n × top_n scale)
