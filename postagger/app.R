...
# Full model list for selectInput: display label = udpipe language key
# Shiny selectInput: choices = c("Label shown to user" = "value returned")
ALL_MODELS <- c(
  # ── Pre-bundled ─────────────────────────────────────────────
  "Arabic (PADT)"               = "arabic-padt",
  "Chinese (GSD)"               = "chinese-gsd",
  "Chinese simplified (GSD)"    = "chinese-gsdsimp",
  ...
  "Portuguese BR (GSD)"         = "portuguese-br",
  ...
  "Ancient Greek (Perseus)"     = "ancient_greek-perseus",
  "Ancient Greek (PROIEL)"      = "ancient_greek-proiel",
  ...
  "Classical Chinese (Kyoto)"   = "classical_chinese-kyoto",
  ...
  "North Sami (Giella)"         = "north_sami-giella",
  "Norwegian Bokmål"            = "norwegian-bokmaal",
  "Norwegian Nynorsk"           = "norwegian-nynorsk",
  "Norwegian Nynorsk (LIA)"     = "norwegian-nynorsklia",
  "Old Church Slavonic"         = "old_church_slavonic-proiel",
  "Old French (SRCMF)"          = "old_french-srcmf",
  "Old Russian (TOROT)"         = "old_russian-torot",
  ...
  "Scottish Gaelic (ARCOSG)"    = "scottish_gaelic-arcosg",
  ...
  "Upper Sorbian (UFAL)"        = "upper_sorbian-ufal",
  "Urdu (UDTB)"                 = "urdu-udtb",
...
CITATION_FOOTER <- tags$div(
  ...
  tags$div(
    ...
    tags$a("→ Tutorial", href = "https://ladal.edu.au/tutorials/postag/postag.html", target = "_blank",
  ...
  tags$blockquote(
    ...
    HTML(paste0(
      ...
      "<em>POSTagger: A browser-based POS tagging and dependency parsing tool</em>. ",
  ...
  tags$details(
    ...
    tags$pre(
      ...
      paste0(
        "@misc{schweinberger2026postagger,\n",
        "  author       = {Schweinberger, Martin},\n",
        "  title        = {POSTagger: A browser-based POS tagging and dependency parsing tool},\n",
        "  year         = {2026},\n",
...
server <- function(input, output, session) {
  ...
  output$dl_params <- downloadHandler(
    ...
    content  = function(file) {
      lines <- c(
        paste0("Tool:                ", "POSTagger — POS Tagging & Dependency Parsing"),
  ...
  output$params_dl_ui <- renderUI({
    downloadButton("dl_params", "⬇ Download parameters (.txt)", class = "ud-dl")
  })
