# ============================================================
#  UDPTagger — LADAL POS Tagging & Dependency Parsing Tool
#  https://ladal.edu.au
#
...
# Full model list from udpipe 2.5 (language → display label)
# Grouped for the dropdown: pre-bundled first, then others
ALL_MODELS <- c(
  # ── Pre-bundled ─────────────────────────────────────────────
...
# Pre-bundled model keys (downloaded at Binder build time)
BUNDLED_MODELS <- c(
...
get_model <- function(language) {
  ...
  # Look for pre-bundled model first
  model_file <- NULL
  if (dir.exists(MODEL_DIR)) {
    candidates <- list.files(MODEL_DIR,
                             pattern = paste0("^", language, ".*\\.udpipe$"),
                             full.names = TRUE)
    if (length(candidates) > 0) model_file <- candidates[1]
  }

  # Fall back to on-demand download into temp dir
  if (is.null(model_file)) {
    dl <- udpipe_download_model(
      language  = language,
      model_dir = tempdir()
    )
    if (dl$download_failed) stop("Model download failed: ", dl$download_message)
...
build_annotated_texts <- function(tagged_df) {
  # One string per document, tokens joined as word_UPOS
  tagged_df |>
...
ui <- fluidPage(
  ...
  # ── Footer ───────────────────────────────────────────────────
  div(class = "ud-footer",
    span("UDPTagger · LADAL · University of Queensland"),
...
server <- function(input, output, session) {
  ...
  output$welcome_box <- renderUI({
    if (input$run_tag == 0) {
      div(class = "ud-info", style = "font-size:.93rem;",
        tags$b("Welcome to UDPTagger."), br(),
        "Upload your plain-text files, select a language model,
         ...
         parsing using the ", tags$a("UDPipe", href = "https://ufal.mff.cuni.cz/udpipe"),
        " toolkit.", br(), br(),
  ...
  output$dl_xlsx <- downloadHandler(
    filename = function()
      paste0("udptagger_", input$language, "_", Sys.Date(), ".xlsx"),
  ...
  output$dl_csv <- downloadHandler(
    filename = function()
      paste0("udptagger_", input$language, "_", Sys.Date(), ".csv"),
    content  = function(file)
  ...
  output$dl_txts <- downloadHandler(
    filename = function()
      paste0("udptagger_tagged_", Sys.Date(), ".zip"),
    content  = function(file) {
      at      <- annotated_texts()
      tmp_dir <- tempfile()
      dir.create(tmp_dir)
