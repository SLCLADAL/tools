...
| 📊 **TopicDetector** | Unsupervised & seeded LDA topic modelling | [Launch](https://binderhub.atap-binder.cloud.edu.au/v2/gh/SLCLADAL/tools-env/main?urlpath=git-pull%3Frepo%3Dhttps%253A%252F%252Fgithub.com%252FSLCLADAL%252Ftools%26urlpath%3Dlab%252Ftree%252Ftools%252Ftopicdetector_launcher.ipynb%26branch%3Dmain) | [Launch](https://mybinder.org/v2/gh/SLCLADAL/tools-env/main?urlpath=git-pull%3Frepo%3Dhttps%253A%252F%252Fgithub.com%252FSLCLADAL%252Ftools%26urlpath%3Dlab%252Ftree%252Ftools%252Ftopicdetector_launcher.ipynb%26branch%3Dmain) |
...
Discover and label topics in a collection of plain-text documents through a two-stage iterative workflow. In Stage 1, unsupervised LDA (`topicmodels::LDA`) explores the corpus at different values of *k* — the top terms per topic are displayed and used to auto-populate a seed dictionary. In Stage 2, seeded LDA (`seededlda::textmodel_seededlda`, `residual = TRUE`) assigns every document to a named topic. Outputs include a top-terms table, a document-topic assignment table with full probability columns, a topic frequency bar chart (count and % per topic), and a topic probability heatmap across all documents. All outputs are downloadable.

- **Input:** one or more `.txt` files
- **Port:** 3846
- **Note:** uses `topicmodels` which initialises slightly slower — the launcher polls for 120 seconds
- **Tutorial:** [Topic Modelling Tutorial](https://ladal.edu.au/tutorials/topicmodels/topicmodels.html)
- **README:** [topicdetector/README.md](topicdetector/README.md)

**Launch URLs**

| Host | URL |
|------|-----|
| ARDC BinderHub ⭐ | `https://binderhub.atap-binder.cloud.edu.au/v2/gh/SLCLADAL/tools-env/main?urlpath=git-pull%3Frepo%3Dhttps%253A%252F%252Fgithub.com%252FSLCLADAL%252Ftools%26urlpath%3Dlab%252Ftree%252Ftools%252Ftopicdetector_launcher.ipynb%26branch%3Dmain` |
| MyBinder.org | `https://mybinder.org/v2/gh/SLCLADAL/tools-env/main?urlpath=git-pull%3Frepo%3Dhttps%253A%252F%252Fgithub.com%252FSLCLADAL%252Ftools%26urlpath%3Dlab%252Ftree%252Ftools%252Ftopicdetector_launcher.ipynb%26branch%3Dmain` |
...
├── topicdetector/
│   ├── app.R                         # TopicDetector Shiny app
│   └── README.md                     # TopicDetector documentation
...
4. The cell polls `localhost:<port>` until the app is ready (up to 90 seconds; 120 seconds for TopicDetector).
...
| `DT` | All tools |
| `zip` | FileRenamer |
| `writexl` | WordFinder, KeywordExtractor, POSTagger, SentimentExplorer, CollocationCalculator, WordWebber, TopicDetector |
| `tidyverse` | WordFinder, KeywordExtractor, POSTagger, TextCleaner, SentimentExplorer |
| `dplyr` | WordWebber, TopicDetector (standalone; others get it via `tidyverse`) |
| `ggplot2` | WordWebber, TopicDetector (standalone; others get it via `tidyverse`) |
| `tibble` | WordWebber, TopicDetector (standalone; others get it via `tidyverse`) |
| `readr` | WordWebber, CollocationCalculator, TopicDetector (CSV download) |
| `quanteda` | WordFinder, KeywordExtractor, WordWebber, TopicDetector |
| `quanteda.textstats` | KeywordExtractor, TopicDetector |
...
| `topicmodels` | TopicDetector (unsupervised LDA) |
| `seededlda` | TopicDetector (seeded/semi-supervised LDA) |
| `SnowballC` | TopicDetector (English stemming) |
| `tidytext` | TopicDetector (tidy LDA beta/gamma matrices) |
